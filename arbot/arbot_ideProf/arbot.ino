  ...
  for (int i = 0; i < maxSteps; i++) {
    for (int m = 0; m < 3; m++) {
      ...
      if (counters[m] >= maxSteps) {
        ...
        digitalWrite(m1.dirPin, m1.dir ? HIGH : LOW);
        digitalWrite(m2.dirPin, m2.dir ? HIGH : LOW);
        digitalWrite(m3.dirPin, m3.dir ? HIGH : LOW);
        delayMicroseconds(100);
        
        if (m1.position != m1.target)
          digitalWrite(m1.stepPin, HIGH);
        
        if (m2.position != m2.target)
          digitalWrite(m2.stepPin, HIGH);

        if (m3.position != m3.target)
          digitalWrite(m3.stepPin, HIGH);
        
        delayMicroseconds(10);

        digitalWrite(m1.stepPin, LOW);
        digitalWrite(m2.stepPin, LOW);
        digitalWrite(m3.stepPin, LOW);
        
        // Correction : mise à jour de la position
        if (m1.dir) m1.position++;
        else m1.position--;

        if (m2.dir) m2.position++;
        else m2.position--;

        if (m3.dir) m3.position++;
        else m3.position--;
